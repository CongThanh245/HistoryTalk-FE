...
function TierCard({ tier, onEdit, onDelete }: TierCardProps) {
  ...
  return (
    <div
      className="relative rounded-2xl border p-6 flex flex-col gap-5 transition-all duration-200 hover:shadow-lg"
      style={{
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          >
            <CreditCardIcon className="w-5 h-5" style={{ color: color.text }} />
          </div>
          <div>
            <h3
      ...
      {/* Divider */}
      <div className="border-t" style={{ borderColor: "var(--card-light-border)" }} />

      {/* Stats grid */}
      <div className="grid grid-cols-3 gap-3">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-1.5">
            <CurrencyCircleDollarIcon className="w-3.5 h-3.5" style={{ color: "var(--accent-gold)" }} />
            <span className="text-[10px] font-semibold uppercase tracking-wide" style={{ color: "var(--content-subtle)" }}>
              Giá
            </span>
          </div>
          <p className="text-base font-extrabold" style={{ color: "var(--content-heading)" }}>
            {tier.amount === 0 ? (
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-1.5">
            <CalendarBlankIcon className="w-3.5 h-3.5" style={{ color: "var(--accent-blue)" }} />
            <span className="text-[10px] font-semibold uppercase tracking-wide" style={{ color: "var(--content-subtle)" }}>
              Thời hạn
            </span>
          </div>
          <p className="text-base font-extrabold" style={{ color: "var(--content-heading)" }}>
            {tier.noMonth} <span className="text-xs font-normal" style={{ color: "var(--content-muted)" }}>tháng</span>
          </p>
        </div>

        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-1.5">
            <CoinsIcon className="w-3.5 h-3.5" style={{ color: "var(--accent-gold)" }} />
            <span className="text-[10px] font-semibold uppercase tracking-wide" style={{ color: "var(--content-subtle)" }}>
              Token
            </span>
          </div>
      ...
      <div className="space-y-6">
        ...
        {!isLoading && (
          <StaffStatsGrid>
            ...
            <StaffStatCard
              ...
              icon={<CurrencyCircleDollarIcon className="h-5 w-5" />}
              tone="amber"
            />
          </StaffStatsGrid>
        )}
        ...
        >
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <Button
              onClick={openCreate}
              className="rounded-xl gap-2 font-semibold border-0"
              style={{ background: "var(--accent-gold)", color: "#fff" }}
            >
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="rounded-2xl border animate-pulse h-56"
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-4">
              <div
                ...
                style={{ background: "rgba(201,162,77,0.10)", border: "1px solid rgba(201,162,77,0.20)" }}
              >
                <PackageIcon className="w-7 h-7" style={{ color: "var(--accent-gold)" }} />
              </div>
              ...
              <Button
                onClick={openCreate}
                size="sm"
                className="rounded-xl gap-2 border-0"
                style={{ background: "var(--accent-gold)", color: "#fff" }}
              >
                <PlusIcon className="h-4 w-4" />
                Tạo gói mới
