 ...
 *
 * Hàm thuần (không phải hook) để có thể gọi lại nhiều lần khi render
 * markdown (mỗi text node một lần) mà không vi phạm rules-of-hooks.
 */
...
/**
 * Bản bọc useMemo của highlightKeywords, dùng khi render trực tiếp
 * một khối text tĩnh (không lồng trong markdown).
 */
export function useKeywordHighlight(
  text: string,
  onSelect: (kw: KeywordData) => void,
): React.ReactNode[] {
  return useMemo(() => highlightKeywords(text, onSelect), [text, onSelect]);
}
